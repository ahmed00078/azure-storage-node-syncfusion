...
const endpointSuffix = process.env.AZURE_ENDPOINT_SUFFIX || 'core.windows.net';
...
app.post('/', async function (req, res) {
    ...
    if (typeof req.body !== 'undefined' && req.body.action === 'read') {
        ...
        // Pass the normalized path to your file retrieval function
        let totalFiles = await getFiles({ ...req, body: { ...req.body, path } });

        let cwdFiles = {
            name: directoryName, // Default if data is null or does not have valid entries
            type: "File Folder",
            filterPath: "",
            size: 0,
            hasChild: true
        ...
        if (Array.isArray(req.body.data) && req.body.data.length > 0 && req.body.data[0] != null) {
            cwdFiles.name = req.body.data[0].name || directoryName; // Use directoryName as a fallback
            cwdFiles.filterPath = req.body.data[0].filterPath || ""; // Default to empty string if not provided
        }
